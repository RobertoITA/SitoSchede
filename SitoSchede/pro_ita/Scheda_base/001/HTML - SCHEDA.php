...
<head>
    <meta charset="UTF-8">
    <?php
    // ... (Il blocco PHP di apertura file e titolo rimane invariato)
    header('Content-Type: text/html; charset=utf-8');
    $fileHandle = fopen("SCHEDA.csv", "r");
    // ... (blocco di lettura CSV)
    if (!$fileHandle) {
...
<?php
if (isset($data)) {
    // ... (La parte di estrazione $logoFile, $title, $schedaImg rimane invariata) ...
...
<table class="header-table">
    <!-- ... (Tutto l'header-table rimane invariato) ... -->
    <tr>
...
<table class="info-table">
    <!-- ... (Tutto l'info-table superiore rimane invariato) ... -->
    <?php
    ...
    <tr>
        <td>
            ...
            function generateFileEntry($filename, $baseDirUrl, $serverFilePathBase) {
                ...
                // *** MODIFICA: Chiama una funzione JS per aprire il modale ***
                echo '<div class="file-entry">';
                echo "<a href='" . htmlspecialchars($filename) . "' download>" . htmlspecialchars($filename) . "</a>";
                
                // La funzione openEmailModal riceve il percorso e il nome del file da allegare
...
<!-- *** NUOVO MODALE PER L'INVIO EMAIL *** -->
<div id="emailModal" class="modal">
    <div class="modal-content">
        ...
        <form id="emailForm">
            ...
            <label for="recipients">Destinatari (separati da virgola o spazio):</label>
            <input type="text" id="recipients" name="recipients" required style="width: 95%; padding: 8px; margin-bottom: 10px;">

            <label for="extraFiles">Aggiungi altri file (nomi separati da virgola, es: file1.pdf, file2.pdf):</label>
...
<script>
    // Percorso relativo allo script PHP intermedio
    ...
    function openEmailModal(filePath, fileName) {
        // Decodifica i nomi per la visualizzazione
    ...
    document.getElementById('emailForm').addEventListener('submit', function(event) {
        ...
        // Disabilita il form per evitare doppi invii
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                ...
                statusDiv.innerText = data.message || 'Email inviata con successo!';
                // Opzionale: setTimeout(closeEmailModal, 3000); // Chiudi dopo 3 secondi
            } else {
                statusDiv.classList.add('status-error');
                statusDiv.innerText = data.message || 'Si è verificato un errore durante l\'invio.';
            }
        })
        .catch(error => {
            submitButton.disabled = false;
